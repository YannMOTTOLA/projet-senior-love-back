...
const optionalNonEmptyString = z
    .preprocess((value) => (Array.isArray(value) ? value[0] : value), z.string().trim().min(1))
    .optional();
...
function buildVisibilityWhere(userId, requestedVisibility) {
...
export async function listEvents(req, res) {
    ...
    const scope = query.scope ?? "recommended";
    const isFiltering = interestsParamProvided ||
    ...
    if (scope === "all") {
        const events = await fetchEvents(buildWhere({ city: query.city, interestIds: requestedInterestIds }));
    ...
    if (!isFiltering) {
        const firstPass = await fetchEvents(buildWhere({ city: currentUser.city, interestIds: currentUserInterestIds }));
...
const createEventBodySchema = z
    ...
    illustration_url: z
        .preprocess((value) => (typeof value === "string" && value.trim() === "" ? undefined : value), z.string().url())
        .optional(),
    visibility: z.enum(["public", "private"]).optional(),
    status: z.enum(["disponible", "complet", "clos"]).optional(),
