 ...
 * Helpers
 */
async function insertMemberInDb(overrides: any = {}) {
...
describe("[BACK OFFICE] Members controller", () => {
    ...
    it("should return one member by id", async () => {
        ...
        const res = await adminRequester.get(
            `/backOffice/members/${user.id}`
        );
    ...
    it("should deactivate a member for 48h", async () => {
        ...
        const res = await adminRequester.patch(
            `/backOffice/members/${user.id}/deactivate-48h`
        );
    ...
    it("should deactivate a member for 1 week", async () => {
        ...
        const res = await adminRequester.patch(
            `/backOffice/members/${user.id}/deactivate-1week`
        );
    ...
    it("should activate a deactivated member", async () => {
        ...
        const res = await adminRequester.patch(
            `/backOffice/members/${user.id}/activate`
        );
    ...
    it("should anonymize and deactivate a user", async () => {
        ...
        const res = await adminRequester.patch(
            `/backOffice/members/${user.id}/delete`
        );
    ...
    it("should return all deactivated members with remaining time", async () => {
        ...
        const res = await adminRequester.get(
            "/backOffice/members/desactivated"
        );
