...
import { visionClient } from "../services/vision.ts";
...
export async function registerUser(req: Request, res: Response) {
  ...
  if (!role) throw new Error("Role 'member' not found in database");
...
export async function getAllInterests(req: Request, res: Response) {
  const interests = await prisma.interest.findMany();
  res.status(201).json(interests);
}

export async function verifyIfMailAlreadyExist(req: Request, res: Response) {
