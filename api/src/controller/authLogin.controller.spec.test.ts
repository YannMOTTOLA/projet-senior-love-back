...
import { generateFakeMember, generateFakeOrganization, httpRequester } from "../../test/index.ts";
...
describe("[POST] /api/auth/login auth/refresh with role", () => {
  ...
  it("should return a JWT with a payload containing the userId and userRole MEMBER", async () => {
    ...
    const decodedToken = jwt.decode(accessToken) as JwtPayload; // "jwt.decode" renvoie le type suivant : JWTPayload (objet) | string | undefined
  ...
  it("should return the refresh token in the set-cookie header on /auth/refresh", async () => {
    ...
    const refreshResponse = await httpRequester.post(
      "/auth/refresh",
      { body: { refreshToken: refreshTokenValue } }, // ou laisser le cookie si le client l’envoie automatiquement
