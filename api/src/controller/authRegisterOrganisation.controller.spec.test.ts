...
import { httpRequester, generateFakeOrganization } from "../../test/index.ts";
...
async function generateFakeOrganizationBody(
  overrides: Partial<RegisterOrganizationBody> = {}
): Promise<RegisterOrganizationBody> {
...
describe("[POST] /api/auth/register/organization", () => {
  it("should register the organization user with the associated organization entry", async () => {
    ...
    assert.equal(
      organization.created_at.toISOString().slice(0, 10),
      BODY.creation_date
    );
  });
